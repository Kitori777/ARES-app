from django.urls import path
from .views import (
    login_view,
    register_view,
    verify_email_view,
    resend_verification_email_view,
    dashboard_view,
    logout_view,
    worksheets_view,
    worksheet_editor_view,
    demo_view,
    import_data_view,
    history_view,
    reports_view,
    network_view,
    addons_marketplace_view,
    helpdesk_view,
    profile_view,
    account_settings_view,
    change_password_view,
    forgot_password_view,
    password_reset_confirm_view,
    profile_settings_api,
    youtube_search_api,
    bug_report_view,
    admin_bug_reports_view,
    admin_addons_view,
    admin_users_panel_view,
)

urlpatterns = [
    path('', login_view, name='login'),
    path('login/', login_view, name='login_explicit'),
    path('accounts/login/', login_view, name='login_accounts_compat'),
    path('password/forgot/', forgot_password_view, name='password_forgot'),
    path('password/reset/', password_reset_confirm_view, name='password_reset_confirm'),
    path('register/', register_view, name='register'),
    path('demo/', demo_view, name='demo'),
    path('verify/', verify_email_view, name='verify_email'),
    path('verify/resend/', resend_verification_email_view, name='resend_verification_email'),
    path('dashboard/', dashboard_view, name='dashboard'),
    path('worksheets/', worksheets_view, name='worksheets'),
    path('worksheets/editor/', worksheet_editor_view, name='worksheet_editor'),
    path('import-data/', import_data_view, name='import_data'),
    path('history/', history_view, name='history'),
    path('reports/', reports_view, name='reports'),
    path('network/', network_view, name='network'),
    path('addons/', addons_marketplace_view, name='addons_marketplace'),
    path('helpdesk/', helpdesk_view, name='helpdesk'),
    path('profile/', profile_view, name='profile'),
    path('profile/account/', account_settings_view, name='account_settings'),
    path('profile/password/', change_password_view, name='change_password'),
    path('api/profile/settings/', profile_settings_api, name='profile_settings_api'),
    path('api/youtube/search/', youtube_search_api, name='youtube_search_api'),
    path('feedback/', bug_report_view, name='bug_report'),
    path('admin-reports/', admin_bug_reports_view, name='admin_bug_reports'),
    path('admin-addons/', admin_addons_view, name='admin_addons'),
    path('admin-users/', admin_users_panel_view, name='admin_users_panel'),
    path('logout/', logout_view, name='logout'),
]
