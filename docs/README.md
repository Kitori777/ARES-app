# ARES — dokumentacja techniczna

Ten folder zbiera pliki tekstowe i README projektu w jednym miejscu.

## Zawartość

- `ADMIN_REPORTS_README.md` — panel zgłoszeń, konto admina i obsługa błędów.
- `SHEET_SHARING_README.md` — udostępnianie arkuszy użytkownikom.
- pozostałe pliki `.md` / `.txt` — notatki techniczne projektu.

## Po wdrożeniu zmian

Po podmianie paczki uruchom migracje:

```bash
uv run python manage.py migrate
```

Następnie wykonaj twarde odświeżenie przeglądarki:

```text
Ctrl + F5
```

W przypadku Rendera najlepiej użyć:
